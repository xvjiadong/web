<template>
    <div class="container">
        ...
        <el-card class="teachcard" v-if="!hideintroduction">
            <div slot="header">
                <span style="font-size: 16px; color: rgb(21, 27, 38)"> 使用流程 </span>
                <br />
                <div>
                    <span style="font-size: 10px; color: #84868c">
                        数据标注平台为一站式数据处理和服务平台，在多人协同标注模式下以任务为基本单元进行标注工作
                    </span>
                </div>
            </div>
            <div class="teachbody">
                <div class="teachbodyitem">
                    ...
                    <div style="text-align: center">
                        <div class="itemtitle">选择标注任务</div>
                        <div class="itemcontent">选择接受的标注任务</div>
                    </div>
                </div>
                <div class="teachbodyitem">
        ...
        ">
            ...
            <div>
                <el-table ref="accept" :data="task">
                    <el-table-column width="180" prop="taskName" label="任务名称">
                        <template slot-scope="scope">
                            <span :style="'color:' + (scope.row.taskName === null ? 'rgb(255,0,0)' : '#606266')">{{
                    scope.row.taskName
                        == null ? '未分配任务' :
                        scope.row.taskName }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column width="180" prop="projectName" label="所属项目"></el-table-column>
                    <el-table-column width="120" prop="version" label="所属版本"></el-table-column>
                    <el-table-column width="100" prop="progress" label="标注进度"></el-table-column>
                    <el-table-column width="180" prop="callType" label="标注类型"></el-table-column>
                    <el-table-column width="180" prop="startTime" label="创建时间">
                        <template slot-scope="scope">
                            <span>{{ scope.row.startTime == null ? '未分配任务' : scope.row.startTime }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column width="180" prop="endTime" label="截止时间">
                        <template slot-scope="scope">
                            <span>{{ scope.row.endTime == null ? '未分配任务' : scope.row.endTime }}</span>
                        </template>
                    </el-table-column>
