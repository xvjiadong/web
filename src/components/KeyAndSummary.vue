            ...
            <div style="border: 1px solid #eee;height: 585px;display: flex;">
                <el-card class="teachcard" style="width: 1050px;">
                    ...
                    <div style="display: flex;justify-content: space-around;align-items: flex-start;">
                        <el-popover ref="popover" placement="bottom" trigger="manual" v-model="visible">
                            <span>确定选择该语句为文本的{{ project.callType === '关键词抽取' ? '关键词' : '摘要' }}吗？</span>
                            <el-button size="mini" @click="entityok" type="primary">确定</el-button>
                            <el-button size="mini" @click="entitycancel">取消</el-button>
                            <div slot="reference" id="text"
                                ...
                                <div v-if="textdata.length > 0" style="width: 830px;height: 480px;">
                                    {{ textdata[nowselect].text }}
                                </div>
                            </div>
                        </el-popover>
                        <div class="mapresult">
                            <div class="mapresultheader">{{ project.callType === '关键词抽取' ? '关键词' : '摘要' }}</div>
                            <div v-if="textdata.length > 0">
                                <div @dblclick="inputshow" v-if="!inputvis">{{ textdata[nowselect].result }}</div>
                                <div v-else>
                            ...
                            <div v-else>
                                <el-empty
                                    :description="'请在文章中选择' + project.callType === '关键词抽取' ? '关键词' : '摘要' + ',或者双击当前标注结果以修改'"></el-empty>
                            </div>
                        </div>
                    </div>
                </el-card>
...
export default {
    ...
    methods: {
        inputok() {
            this.textdata[this.nowselect].result = this.inputstr
            this.inputcancel()
        },
        inputcancel() {
            this.inputstr = ""
        ...
        inputshow() {
            this.inputstr = this.textdata[this.nowselect].result
            this.inputvis = true
        },
        entityok() {
            this.textdata[this.nowselect].result = this.selectstr
            this.entitycancel()
        },
        entitycancel() {
            this.selectentity = ""
    ...
    mounted() {
        ...
        text.addEventListener("mouseup", (e) => {
            ...
            let timer = setTimeout(() => {
                ...
                popover.popperElm.style.top = e.clientY + "px";
