            ...
            <div style="border: 1px solid #eee;height: 585px;display: flex;">
                <el-card class="teachcard" style="width: 1050px;">
                    ...
                    <div style="display: flex;justify-content: space-around;align-items: flex-start;">
                        <el-popover placement="bottom" ref="popover" trigger="manual" v-model="visible">
                            <span>确定选择该语句为文本的答案吗？</span>
                            <el-button size="mini" @click="entityok" type="primary">确定</el-button>
                            <el-button size="mini" @click="entitycancel">取消</el-button>
                            <div id="text" slot="reference"
                                ...
                                <div v-if="textdata.length > 0" style="width: 830px;height: 480px;">
                                    {{ textdata[0].result[nowselect].content }}
                                </div>
                            </div>
                        </el-popover>
...
export default {
    ...
    methods: {
        inputok() {
            this.textdata[this.nowselect].answer = this.inputstr
            this.inputcancel()
        },
        inputcancel() {
            this.inputstr = ""
        ...
        inputshow() {
            this.inputstr = this.textdata[this.nowselect].answer
            this.inputvis = true
        },
        entityok() {
            this.textdata[this.nowselect].answer = this.selectstr
            this.entitycancel()
        },
        entitycancel() {
            this.selectentity = ""
        ...
        save(index) {
            let jsondata = JSON.stringify(this.textdata[0])
            let file = new FormData()
            const blob = new Blob([jsondata]);
            file.set("file", blob, "data.json")
            file.set("id", this.textdata[0].id)
            ...
            if (this.page === 1 && index === -1) {
                ...
                return
            } else if ((index === this.textdata[0].result.length || index == -1)) {
        ...
        choose(item) {
            this.textdata[0].result[this.nowselect].link = item.label
        },
        goBack() {
