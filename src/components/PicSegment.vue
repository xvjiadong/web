	...
	<el-card>
      ...
      ">
			<div style="display: flex; justify-content: left; align-items: center">
				...
				<span>{{ project.projectName }}->{{ project.version }}->{{
					project.callType
				}}</span>
			</div>
			<span @click="push" style="
		...
		<div class="main" v-loading="loading">
			<el-card class="teachcard">
				<div slot="header" class="operation">
					<div class="button-wrap">
						...
						<el-tooltip content="圆标注" placement="top-start">
							<div class="toolblock" :class="{ modeing: mode === 'CIRCLE' }">
								<el-button type="text" class="el-icon-orange button"
									@click="setMode('CIRCLE')"></el-button>
							</div>
						</el-tooltip>
						...
						<div v-if="segvis">
							<span class="segment" @click="segok">确认分割</span>
						</div>
						<div v-if="!segvis && type == 'POINT'">
							<span class="segment" @click="pointok">确认分割</span>
						</div>
					</div>
					<div style="display: flex; align-items: center; width: 50%">
						...
						<span v-if="!textpromptdisable" @click="
					text_to_segment(showlist[nowselect].url, project.versionId)
            ...
            ">
						...
						<el-button size="mini" @click="Revoke"> 取消 </el-button>
					</div>
								...
								<el-card>
									<div slot="header" style="display: flex; justify-content: space-between">
										<span style="font-size: 20px">
											...
											<span v-if="!item.edit" @dblclick="
					item.edit = true;
				if (!item.vis) appearfeature(item, index);
				">
...
export default {
	...
	methods: {
		...
		text_to_segment(url) {
			if (this.textprompt) {
				this.socket.emit("textprompt_to_segment", {
					url: url,
					label: this.seglabels,
		...
		changelabel(item, index) {
			...
			const polygontext = new AILabel.Text(item.textid, {
				text: item.textInfo.text,
				position: item.textInfo.position,
				offset: item.textInfo.offset,
			});
		...
		addlabel() {
			if (this.seglabels.includes(this.newlabel) || this.newlabel === "") {
				this.$notify.error({
					title: "添加失败",
					message: this.seglabels.includes(this.newlabel)
						? "标签重复"
						: "标签为空",
					duration: 3000,
				});
		...
		pointok() {
			...
			this.pointlist.forEach((point) => {
				this.gFirstFeatureLayer.removeFeatureById(point.id);
			});
			this.pointlist.splice(0);
		},
		segok() {
			axios
				.post("http://localhost:5000/api/segment_manual", {
					point: this.pointlist,
					box: this.rect,
					url: this.gMap.layers[0].imageInfo.src,
				})
				.then((res) => {
					let id = Date.now() + "";
					if (!this.segvis) {
		...
		keepdraw(markdata, url) {
			...
			if (mattinglist.length > 0) {
				axios
					.post("http://localhost:5000/api/matting", {
						url: url,
						mattinglist: mattinglist,
					})
					.then((res) => {
						console.log(this.nowpicdata);
						res.data.forEach((item) => {
							this.nowpicdata[
								this.nowpicdata.findIndex((item2) => item2.id == item.id)
							].base64str = item.code;
						});
					})
					.catch((e) => {
		...
		//汇总框选向后台提交//
		together(num) {
			if (this.generate_manual) return;
			...
			axios
				.put("http://120.26.142.114:10010/dataset/call", file, {
		...
		ok() {
			...
			if (this.type === "RECT") {
				let textid =
					this.gFirstFeatureLayer.getAllFeatures()[
						this.gFirstFeatureLayer.getAllFeatures().length - 1
					].id +
					"-" +
					0;
				const recttext = new AILabel.Text(textid, {
					text: this.Recognizetextcontent,
					position: { x: this.data2.x, y: this.data2.y },
					offset: {
						x: 1,
						y: this.data2.y < 19 ? -this.data2.y - this.data2.height - 25 : 0,
					},
				});
				...
				let id = Date.now() + "";
				this.gFirstFeatureLayer.removeFeatureById(
					this.gFirstFeatureLayer.getAllFeatures()[
						this.gFirstFeatureLayer.getAllFeatures().length - 1
					].id
				);
				axios
					.post("http://localhost:5000/api/getmask", {
						point: this.data2,
						url: this.showlist[this.nowselect].url,
						type: "POLYGON",
					})
					.then((res) => {
						let color = `rgb(${Math.floor(Math.random() * (255 - 0 + 1)) + 0},${Math.floor(Math.random() * (255 - 0 + 1)) + 0
						...
						this.addFeature(
							...
							this.segtext !== ""
								? color
								: this.seglabels[this.Recognizetextcontent].color,
							res.data,
							{
								text:
									this.segtext == ""
										? this.seglabels[this.Recognizetextcontent].label
										: this.segtext,
								position: { x: sum_x, y: sum_y },
								offset: { x: 1, y: 0 },
							}
						);
						let textid = id + "-" + 0;
						const polygontext = new AILabel.Text(textid, {
							text: this.seglabels[this.Recognizetextcontent].label,
							position: { x: sum_x, y: sum_y },
							offset: { x: 1, y: 0 },
						});
						this.tagtextLayer.addText(polygontext);
				...
				const polygontext = new AILabel.Text(textid, {
					text:
						this.segtext == ""
							? this.seglabels[this.Recognizetextcontent].label
							: this.segtext,
					position: { x: this.data2[num].x, y: this.data2[num].y },
					offset: { x: 1, y: 0 },
				});
				this.tagtextLayer.addText(polygontext);
				this.Recognizetextcontent = "";
							...
							res.data,
							{
								text:
									this.segtext == ""
										? this.seglabels[this.Recognizetextcontent].label
										: this.segtext,
								position: { x: this.data2.cx, y: this.data2.cy },
								offset: { x: 0, y: 0 },
							}
						);
						let textid = id + "-" + 0;
				...
				this.gFirstFeatureLayer.removeFeatureById(this.storage.id);
				let sum_x = this.storage.point.reduce(
					(acc, current) => acc + current.x,
					0
				);
				let sum_y = this.storage.point.reduce(
				...
				const polygontext = new AILabel.Text(textid, {
					text: this.Recognizetextcontent,
					position: { x: this.data2.start.x, y: this.data2.start.y },
					offset: { x: 1, y: 0 },
				});
				this.tagtextLayer.addText(polygontext);
				this.Recognizetextcontent = "";
		...
		la(e) {
			if (this.type === "POINT") {
				...
				let a = setTimeout(() => {
					...
					if (
						this.type === "CIRCLE" ||
						(this.type === "LINE" && this.linenumber % 2 === 0)
					) {
						if (!this.vis) {
							const popover = this.$refs.popover;
							let timer = setTimeout(() => {
		...
		addFeature(
			...
			//点
				...
				const gFirstFeaturePoint = new AILabel.Feature.Point(
					...
					{ name }, // props
					{
						fillStyle: this.rightclick ? "rgb(255,0,0)" : "rgb(0,255,0)",
						zIndex: 5,
						lineWidth: 255,
					} // style
				);
			...
			if (
				rlecode !== null &&
				textInfo !== null &&
				!isappear & (base64str == null)
			) {
				axios
					.post("http://localhost:5000/api/matting", {
						url: this.showlist[this.nowselect].url,
						mattinglist: [{ code: rlecode, id: id }],
					})
					.then((res) => {
			...
			gMap.events.on("drawDone", (type, data) => {
				...
				if (type == "CLEARMASK" || type == "DRAWMASK") {
					...
					if (
						...
						data.width > 10 &&
						data.height > 10 &&
						500 - data.x - data.width > 0 &&
						data.y + data.height < 375 &&
						data.x > 0 &&
						data.y > 0
					) {
						this.power = true;
					...
					if (item.id.split("-")[0] === id) {
						switch (feature.type) {
							...
							case "POLYGON": {
								...
								let position = {
									x: shape.points[num].x,
									y: shape.points[num].y,
								};
								item.updatePosition(position);
								break;
							}
		...
		Revoke() {
			...
			for (let i = this.allText.length - 1; i >= 0; i--) {
				if (
					this.allText[i].id.split("-")[0] !==
					this.allFeatures[this.allFeatures.length - 1].id
				) {
					break;
				} else {
					this.allText.pop();
		...
		changepic(item) {
			...
			const tagtextLayer = new AILabel.Layer.Text(
				"tagtext-layer",
				{ name: "as" },
				{ zIndex: 10, opacity: 1 }
			);
			...
			if (item.mark) {
				...
				axios
					...
					.then((res) => {
						...
						this.keepdraw(res.data, item.url);
					})
					.catch((e) => {
						console.log(e);
					});
		...
		getdata(page, num) {
			axios
				.post(
					"http://120.26.142.114:10010" +
					(this.project.identity == 0
						? "/dataset/task"
						: "/dataset/admin/select"),
					{
						version: this.project.versionId,
						page: page,
						number: 5,
						current: page,
						pageSize: 5,
				...
				.then((res) => {
					if (
						(this.project.identity == 0 && res.data.data.length === 0) ||
						(this.project.identity == 1 &&
							res.data.data.datasetCallNewVOS.length == 0)
					) {
			...
			axios
				.get("http://120.26.142.114:10010/task/label/" + this.project.versionId)
				.then((res) => {
	...
	mounted() {
		...
		this.project.versionId = parseInt(this.project.versionId);
		this.getdata(1, 1);
