    ...
    methods: {
        ...
        keepdraw(markdata) {
            markdata.forEach((item, index) => {
                let a = this.seglabels.findIndex(item2 => item2.label == item.textInfo.text)
                if (a !== -1) {
                    this.nowpicdata[index].color = this.seglabels[a].color
                } else {
                    this.seglabels.push({ label: item.textInfo.text, color: item.color })
                }
                if (item.vis) {
                    this.addFeature(item.shape, item.type, item.id, a === -1 ? item.color : this.seglabels[a].color)
